...
##############################################################
#' Simulate a seedDispRcvFast process on a landscape.
#'
#' Simulate seed dispersal using user defined function. This is a "receiving pixel" focused dispersal approach.
#' It is the "potentially receiving" cell that looks around itself for potential seed sources. If it finds
#' a single seed source, that passes the probability function described by the dispersalFn, then the
...
#' @author Eliot McIntire
#'
#' @name seedDispRcvFast
#' @aliases seedDispRcvFast
#' @rdname seedDispRcvFast
setGeneric("seedDispRcvFast", function(dtSrc, dtRcv, pixelGroupMap,
                                       dispersalFn=WardFast,
                                       b=0.01, k=0.95,
                                       plot.it=FALSE, ...) {
  standardGeneric("seedDispRcvFast")
})
...
#'
#' seedRcv <- hab>5
#' system.time(seeds <- seedDispRcvFast(seedSrc, seedRcv=seedRcv,
#'   maxDist=250, plot.it=TRUE))
#' seedRcvRaster <- raster(seedSrc)
#' if(length(seeds)>0) {
#'   seedRcvRaster[seeds] <- 1
#'   Plot(seedRcvRaster, cols="black")
#' }
setMethod("seedDispRcvFast",
          signature(dtSrc="data.table"),
          definition = function(dtSrc, dtRcv, pixelGroupMap, dispersalFn,
                                b, k,
                                plot.it=FALSE, ...) {
            cellSize=unique(res(pixelGroupMap))

            seedSourceMap <- pixelGroupMap %in% unique(seedSource$pixelGroup)
            seedReceiveMap = pixelGroupMap %in% unique(seedReceive$pixelGroup)

            seedSrc=seedSourceMap
            seedRcv=seedReceiveMap
            ...
            lociReturn <- data.table(fromInit=seedRcv,key="fromInit")
            seedsArrived <- data.table(fromInit=numeric(),species=character(),key=c("fromInit","species"))
            ...
            sc <- species %>%
              mutate(speciesCode=as.numeric(as.factor(species))-1) %>%
              select(species, speciesCode, seeddistance_eff, seeddistance_max) %>%
            ...
            seedRcv <- Which(!is.na(seedReceiveMap), cells=TRUE)

            speciesComm <- function(num){
              indices <- lapply(strsplit(intToBin(num), split=""),
            ...
            potentials <- data.table("fromInit"=seedRcv,"RcvCommunity"=seedReceiveMap[seedRcv],
                                     key="fromInit")
            potentials[,from:=fromInit]
            setkey(potentials,"from", "fromInit")


            while (length(seedRcv) & ((n-cellSize)<=ultimateMaxDist)) { # while there are active cells and less than maxDistance
              ...
              if(n>cellSize) {
                # replace "from" column with the values from the previous "to" column
                potentials[,`:=`(from=NULL)][,from:=to][,to:=NULL]
                setkey(potentials,"from", "fromInit")
              }
              potentials <- potentials[adjCells, allow.cartesian=TRUE]
              ...
              # Section - omit cells based on one of three criteria
              # 1. Can't spread backwards, within a cluster
              # keep only neighbours that have not been seedDispRcvFast to yet, within each
              # cluster. This means that a cell can't seedDispRcvFast backwards, but two different
              # clusters can be on the same cell
              potentials <- potentials[from!=to,.SD,by="fromInit"]
              ...
              # Go to species level
              sC <- speciesComm(unique(speciesRcvPool$speciesRcvPool))
              sC <- speciesComm(unique(potentials$RcvCommunity))
              setkey(sC, RcvCommunity)
              setkey(potentials, RcvCommunity)
              a = sC[potentials, allow.cartesian=TRUE]
              setkey(a, speciesCode)
              specPotentials <- sc[a]
              specPotentials <- specPotentials[(dis <= pmin(n,maxDist)),]

              # for speeding up. If no pixels within the doughnut are a seed source,
              #  just skip next block
              potentialsWithSeed <- as.logical(seedSrcVec[specPotentials[,to]])
              if(any(potentialsWithSeed)) {

                potentialsWithSeedDT  <- specPotentials[potentialsWithSeed,]
                nr <- NROW(potentialsWithSeedDT)
                #setkey(potentialsWithSeedDT, "fromInit")

                potentialsWithSeedDT[,receivesSeeds:=runif(nr)<eval(dispersalFn)]
                receivedSeeds <- potentialsWithSeedDT[,any(receivesSeeds), by=c("fromInit,species")]

                #drop any that received seeds from potentials, as they are now in lociReturn
                if(NROW(receivedSeeds[V1==TRUE])>0) {
                  seedsArrived <- rbindlist(list(seedsArrived,lociReturn[receivedSeeds[V1==TRUE]][,V1:=NULL]))
                  setkey(seedsArrived, fromInit, species)
                  setkey(specPotentials, fromInit, species)
                  specPotentials <- specPotentials[!seedsArrived]
                }
            ...
            seedsArrived %<>% rename(pixelIndex=fromInit)
            return(seedsArrived)
